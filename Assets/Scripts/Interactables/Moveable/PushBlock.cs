...
namespace IT.Interactables.Moveable
{
    ...
    {
        public override InteractionType Kind => InteractionType.Move;
        ...
        private void Awake()
        {
            ...
            UpdateLayerName();
            _targetCheck = GetComponentInChildren<KeyPortOverlap>();

            _obstructionMask = 0;
            _obstructionMask |= 1 << LayerIndex.SlidableObstruction;   // walls/boundaries (same place SlidableBase expects them)
            _obstructionMask |= 1 << LayerIndex.Interactable;          // other blocks, doors
            _obstructionMask |= 1 << LayerIndex.Locked;
            // _obstructionMask |= 1 << LayerIndex.Enemy;              // add your actual enemy layer here
        }
