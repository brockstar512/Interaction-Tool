...
namespace IT.Items.GrapplingHook
{
    ...
    public class RopeBridge : MonoBehaviour
    {
        ...
        private IAllOverlap<Collider2D> overlappingDepth { get; set; }

        private void Awake()
        {
            overlappingDepth = GetComponent<IAllOverlap<Collider2D>>();
        }
        ...
        public async Task Connect(Vector3 start, Vector3 end)
        {
            ...
            BuildWalls(_depthsCol,GameUtilities.GetDirectionFromTwoPoints(start, end));
        }
        ...
        {
            if (other.gameObject.layer == LayerIndex.Player)
            {
                foreach (var col in _depthsCol)
                {
