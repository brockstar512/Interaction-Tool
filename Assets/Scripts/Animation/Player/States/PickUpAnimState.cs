...
namespace IT.Animation.Player.States
{
    ...
    public class PickUpAnimState : AnimStateBase
    {
        ...
        {
            Transform originHolder = state.GetComponentInChildren<ItemAnchorPoint>().transform;
            if (state.item is ThrowableBase throwItem)
            {
