...
namespace IT.Overlap
{
    ...
    public class OverlapMoveDamageCheck : MonoBehaviour, IDamage
    {
        ...
        private void RemoveDetectionLayers()
        {
            ...
            // detectionLayer &= ~(1 << Layers.Interacting);

            // detectionLayer &= ~(1 <<Layers.Interactable);
            detectionLayer &= ~(1 << Layers.Player);
            detectionLayer &= ~(1 << Layers.KeyPort);
            detectionLayer &= ~(1 << Layers.TargetOverlap);
        ...
        private void SlideCollision(Collider2D collision)
        {
            ...
            if(collision.gameObject.layer != Layers.SlidableObstruction)
            {
                //Debug.Log("Emergency stop");
                //Debug.Break();
                //_emergencyStop?.Invoke();
            }
            else if (collision.gameObject.layer == Layers.SlidableObstruction)
            {
                //Debug.Log("Emergency stop");
                //_emergencyStop?.Invoke();
               // Debug.Break();
