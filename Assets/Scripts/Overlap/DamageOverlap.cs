...
namespace IT.Overlap
{
    ...
    public class DamageOverlap : MonoBehaviour
    {
        ...
        private void RemoveDetectionLayers()
        {
            ...
            // detectionLayer &= ~(1 << LayerIndex.Interacting);

            // detectionLayer &= ~(1 <<LayerIndex.Interactable);
            detectionLayer &= ~(1 << LayerIndex.Player);
            detectionLayer &= ~(1 << LayerIndex.KeyPort);
            detectionLayer &= ~(1 << LayerIndex.TargetOverlap);
        ...
        {
            IDamageable collidedSubject = collision.GetComponent<IDamageable>();
            ...
            if(collision.gameObject.layer != LayerIndex.SlidableObstruction)
            {
                //Debug.Log("Emergency stop");
                //Debug.Break();
                //_emergencyStop?.Invoke();
            }
            else if (collision.gameObject.layer == LayerIndex.SlidableObstruction)
            {
                //Debug.Log("Emergency stop");
                //_emergencyStop?.Invoke();
               // Debug.Break();
